      ...
      <div className="container mt-5">
        <div className="card shadow">
          ...
          <div className="card-body">
            ...
            <p>
              <strong>Amount Paid:</strong> KES {paymentDetails.amount_paid}
            </p>
            <p>
              <strong>Transaction Code:</strong>{" "}
              {paymentDetails.transaction_code || "N/A"}
            </p>
            <p>
              <strong>Date:</strong>{" "}
              {new Date(paymentDetails.created_at).toLocaleDateString()}
            </p>
            {paymentDetails.loan > 0 && (
              <p>
                <strong>Loan:</strong> KES {paymentDetails.loan}
              </p>
            )}
            {paymentDetails.savings > 0 && (
              <p>
                <strong>Savings:</strong> KES {paymentDetails.savings}
              </p>
            )}
            {paymentDetails.insurance > 0 && (
              <p>
                <strong>Insurance:</strong> KES {paymentDetails.insurance}
              </p>
            )}
            {paymentDetails.operations > 0 && (
              <p>
                <strong>Operations:</strong> KES {paymentDetails.operations}
              </p>
            )}
            {paymentDetails.matatu_plate && (
              <p>
                <strong>Matatu Plate Number:</strong>{" "}
                {paymentDetails.matatu_plate}
              </p>
            )}
          </div>
