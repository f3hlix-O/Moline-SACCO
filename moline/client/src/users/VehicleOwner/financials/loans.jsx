...
function LoanApplication() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      setLoading(true);
      try {
        const [ud, mats, pending, usrs] = await Promise.all([
          fetchUserData(),
          fetchMatatus(),
          fetchPendingLoans(),
          fetchUsers(),
  ...
  const handleApplyLoan = (matatu_id, type) => {
    ...
    const emergencyLoanPending = pendingLoans.some(
      (loan) => loan.loan_type === "emergency",
    );
    ...
    if (type === "normal") {
      const hasPendingLoan = pendingLoans.some(
        (loan) => loan.matatu_id === matatu_id,
      );
      const totalSavings = Number(foundMatatu?.total_savings || 0);
      const loanAmountDue = Number(foundMatatu?.loan || 0);
      if (hasPendingLoan || loanAmountDue > totalSavings) {
        Swal.fire({
          icon: "error",
          title: "Unable to Apply",
          text: "There is a pending loan or insufficient savings for this vehicle.",
          toast: true,
          position: "top-end",
          showConfirmButton: false,
          timer: 3000,
  ...
  const handleSubmitApplication = async (event) => {
    ...
    if (loanType === "normal") {
      ...
      if (loanAmount > Number(selectedMatatu?.total_savings || 0)) {
        Swal.fire({
          icon: "error",
          title: "Invalid Loan Amount",
            ...
            {loanType === "emergency" && (
              <>
                <Form.Group controlId="guarantorSearch">
                  ...
                  <Form.Control
                    type="text"
                    onChange={handleGuarantorSearch}
                    placeholder="Search for guarantors by name"
                  />
                </Form.Group>
