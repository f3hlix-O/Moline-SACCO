...
const fetchMatatus = async () => {
  try {
    const { data } = await axiosInstance.get("/matatus");
    return data;
  } catch (error) {
...
const fetchMatatuDetais = async () => {
  try {
    const { data } = await axiosInstance.get("/reports/matatuDetails");
    return data;
  } catch (error) {
...
const assignDrivers = async (matatuId, drivers, setMatatus) => {
  const { value: selectedDriver } = await Swal.fire({
    ...
    inputOptions: drivers.reduce((options, driver) => {
      options[driver.driver_id] = driver.full_name;
      return options;
    }, {}),
  ...
  try {
      ...
      html: `
                ...
                <div class="form-group">
                    ...
                    <select id="insurance" class="form-control">
                      <option value="active" ${String(matatuData.insurance_status).toLowerCase() === "active" ? "selected" : ""}>active</option>
                      <option value="inactive" ${String(matatuData.insurance_status).toLowerCase() === "inactive" ? "selected" : ""}>inactive</option>
                    </select>
                </div>
            `,
      showCancelButton: true,
    ...
    if (formValues) {
      const updateResponse = await axiosInstance.post(
        `/matatus/${matatuData.matatu_id}/update`,
        formValues,
      );
      ...
      if (updateResponse.status >= 200 && updateResponse.status < 300) {
        Swal.fire(
          "Changes Saved",
          "Matatu information updated successfully",
          "success",
        );
        const updatedMatatus = await fetchMatatuDetais();
        setMatatus(updatedMatatus);
