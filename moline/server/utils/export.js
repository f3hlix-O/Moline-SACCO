...
async function generatePDF(data) {
  ...
  data.forEach((item) => {
    const driverName =
      ...
      `${item.driver_first_name || ""} ${item.driver_last_name || ""}`.trim();
    const ownerName = `${item.owner_first_name} ${item.owner_last_name}`;
    page.drawText(
      `Matatu: ${item.number_plate}, Driver: ${driverName}, Owner: ${ownerName}, Status: ${item.status}`,
      {
        x: 50,
        y,
        size: 12,
...
async function generateDOCX(data) {
  ...
  data.forEach((item) => {
    const driverName =
      ...
      `${item.driver_first_name || ""} ${item.driver_last_name || ""}`.trim();
    const ownerName = `${item.owner_first_name} ${item.owner_last_name}`;
    const paragraph = new Paragraph().addRun(
      new TextRun(
        `Matatu: ${item.number_plate}, Driver: ${driverName}, Owner: ${ownerName}, Status: ${item.status}`,
      ).bold(),
    );
